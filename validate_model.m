% validate_model.m
function [avg_loss, accuracy, precision, recall] = validate_model(valid_embs, Y_valid, A, B, C_mat, Delta, W, b, epsilon)
% VALIDATE_MODEL Evaluates the model on the validation set.
%
% Inputs:
%   - valid_embs: Cell array of validation sequences, each cell contains a matrix of embeddings [D x T].
%   - Y_valid: Cell array of one-hot encoded labels for validation sequences [C x T].
%   - A, B, C_mat, Delta, W, b: Trained model parameters.
%   - epsilon: Small value for numerical stability.
%
% Outputs:
%   - avg_loss: Average cross-entropy loss over the validation set.
%   - accuracy: Classification accuracy on the validation set (in [0, 1]).
...
%   - recall: Recall for each class (1 x C).
    ...
    total_predictions = 0;
    num_classes = size(Y_valid{1}, 1);  % Assuming Y_valid is not empty
    ...
    for j = 1:length(valid_embs)
        % Extract the input sequence and labels
        X_seq = valid_embs{j};    % Shape: [D x T]
        Y_seq = Y_valid{j};       % Shape: [C x T]
